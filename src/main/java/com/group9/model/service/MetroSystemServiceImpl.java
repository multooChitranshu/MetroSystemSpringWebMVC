package com.group9.model.service;
...
import com.group9.model.persistence.MetroCardDAO;
import com.group9.model.persistence.MetroStationDAO;
import com.group9.model.persistence.TransactionDAO;
...
public class MetroSystemServiceImpl implements MetroSystemService {
	...
	public boolean swipeIn(long cardId,int sourceStationId) {
		...
		long transactionId=transactionDAOImpl.getLastTransactionId()+1;
		if(metroStationDAOImpl.isValidStation(sourceStationId))
			if(cardBalance(cardId)>=20)
				return transactionDAOImpl.swipeIn(transactionId,cardId, sourceStationId);
		return false;
...
//		If more time taken, then charge Rs. 30 for every extra 30 mins
	...
	public List<MetroStation> getAllStations() {
		return metroStationDAOImpl.getAllStations();
	}

	@Override
	public int getStationIdFromName(String stationName) {
		return metroStationDAOImpl.getStationIdFromName(stationName);
	}

	@Override
	public MetroCard getCardByID(long cardId) {
		return metroCardDAOImpl.searchCard(cardId);
	}
}
